...
class PurchaseOrder(models.Model):
    ...
    receipt_reminder_email = fields.Boolean(
        "Receipt Reminder Email", compute="_compute_receipt_reminder_email"
    )
    reminder_date_before_receipt = fields.Integer(
        "Days Before Receipt", compute="_compute_receipt_reminder_email"
    )
    ...
    def _get_orders_to_remind(self):
        ...
        return self.search(
            ...
            lambda p: p.partner_id.with_company(p.company_id).receipt_reminder_email
    ...
    def _get_gross_price_unit(self):
        ...
        if self.taxes_id:
            ...
            price_unit = self.taxes_id.with_context(round=False).compute_all(
                price_unit, currency=self.order_id.currency_id, quantity=qty
            )["total_void"]
