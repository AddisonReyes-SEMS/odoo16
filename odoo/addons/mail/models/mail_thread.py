...
class MailThread(models.AbstractModel):
    ...
    def message_post(self, *,
        ...
        #     message_type: {message_type} ~> {type(message_type)}
        #     email_from: {email_from} ~> {type(email_from)}
        #     author_id: {author_id} ~> {type(author_id)}
        #     parent_id: {parent_id} ~> {type(parent_id)}
        #     subtype_xmlid: {subtype_xmlid} ~> {type(subtype_xmlid)}
        #     subtype_id: {subtype_id} ~> {type(subtype_id)}
        #     partner_ids: {partner_ids} ~> {type(partner_ids)}
        #     attachments: {attachments} ~> {type(attachments)}
        #     attachment_ids: {attachment_ids} ~> {type(attachment_ids)}
        # """)
